...
class GaussTRDecoder(nn.Module):
    ...
    def __init__(self, embed_dims, num_layers, return_intermediate, layer_cfg, post_norm_cfg):
        super().__init__()
        ...
        self.reg_branches = nn.ModuleList([
            MLP(self.embed_dims, self.embed_dims, 3, 3)
            for _ in range(num_layers)
        ])
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.reg_branches is not None:
                tmp_reg_preds = self.reg_branches[lid](query)[..., :2]
...
class GaussTRDecoderLayer(nn.Module):
    ...
    def __init__(self, self_attn_cfg, cross_attn_cfg, ffn_cfg, norm_cfg=dict(type='LN')):
        super().__init__()
        self.self_attn = MultiheadAttention(batch_first=True, **self_attn_cfg)
        self.cross_attn = MultiScaleDeformableAttention(batch_first=True, **cross_attn_cfg)
