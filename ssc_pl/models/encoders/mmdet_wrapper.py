...
class MMDetWrapper(nn.Module):
    ...
    def __init__(self,
        ...
        self.use_encoder_deform_att = use_encoder_deform_att
        ...
        self.conv_projs = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(self.hidden_dims * scale // 4, self.hidden_dims, 1),
                nn.BatchNorm2d(self.hidden_dims),
                nn.ReLU(inplace=True),
                nn.Conv2d(self.hidden_dims, self.hidden_dims, 1),
            ) for scale in scales
        ])
    ...
    def forward(self, x):
        ...
        #     print(f'feat[{i}].shape: {feats[i].shape}')

        if self.use_encoder_deform_att:
            mask_feat, _, multi_scale_feats = self.model.panoptic_head.pixel_decoder.forward_features(
                feats, masks=None)
            # print(f'len(multi_scale_feats): {len(multi_scale_feats)}')
            # for i in range(len(multi_scale_feats)):
            #     print(f'multi_scale_feats[{i}].shape: {multi_scale_feats[i].shape}')
            feats = (feats[0], *multi_scale_feats[:2])
        else:
            feats = feats[:3]
            feats = [proj(feat) for proj, feat in zip(self.conv_projs, feats)]
