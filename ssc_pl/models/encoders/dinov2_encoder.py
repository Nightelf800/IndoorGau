...
import torch
from ..dinov2 import dinov2_vitb14, dinov2_vitg14, dinov2_vitl14, dinov2_vits14, dinov2_vitb14_reg
...
class Dinov2Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        if model_name == "dinov2_vits14":
            ...
            self.model = dinov2_vitb14_reg(pretrained=True if checkpoint_path is None else False)
            self.scaleup_model = dinov2_vitb14_reg(pretrained=False)
        else:
    ...
    def forward(self, imgs, scaleup_imgs=None):
        ...
        dino_feats = self.model.forward_features(imgs)

        x = dino_feats['x_norm_patchtokens']
        feat_ori = x.mT.reshape(B, -1, h // self.patch_size, w // self.patch_size)


        if self.use_clstoken:
            cls_token = dino_feats['x_norm_clstoken']
        ...
        #     scale_h, scale_w = scaleup_imgs.shape[2:]
        ...
        #         scaleup_imgs = nn.functional.interpolate(scaleup_imgs, size=(new_scaleup_h, new_scaleup_w), mode='bilinear', align_corners=False)
        ...
        #             scale_x = x
        #         else:
        #             scale_x = self.DANet(x, feats_ori[0])
        ...
        # print(f'feat_ori.shape: {feat_ori.shape}')
